...
func (state *RuntimeState) SendRequestemail(username string, groupnames []string,
	...
	for _, entry := range groupnames {
		description, err := state.user.GetDescriptionvalue(entry)
		if err != nil {
			log.Println(err)
			return err
		}
		if description == "self-managed" {
			usersEmail, err := state.user.GetEmailofusersingroup(entry)
			if err != nil {
				log.Println(err)
				return err
...
func (state *RuntimeState) sendApproveemail(username string,
	...
	userEmail, err := state.user.GetEmailofauser(username)
	if err != nil {
		log.Println(err)
		return err
	}
	for _, entry := range userPair {
		...
		otheruserEmail, err := state.user.GetEmailofauser(requesteduser)
		if err != nil {
			log.Println(err)
			return err
		}
