...
func (state *RuntimeState) IndexHandler(w http.ResponseWriter, r *http.Request) {
	...
	if state.user.UserisadminOrNot(username) == true {
...
func (state *RuntimeState) MygroupsHandler(w http.ResponseWriter, r *http.Request) {
	...
	userGroups, err := state.user.GetgroupsofUser(username)
	if err != nil {
		log.Println(err)
		http.Error(w, fmt.Sprint(err), http.StatusInternalServerError)
		return
	...
	if state.user.UserisadminOrNot(response.UserName) {
...
func (state *RuntimeState) pendingRequests(w http.ResponseWriter, r *http.Request) {
	...
	sidebarType := "sidebar"
	if state.user.UserisadminOrNot(username) {
		sidebarType = "admins_sidebar"
	}
	if groupnames == nil {
...
func (state *RuntimeState) AddmemberstoGroup(w http.ResponseWriter, r *http.Request) {
	...
	for _, member := range strings.Split(members, ",") {
		...
		groupinfo.Member = append(groupinfo.Member, state.user.CreateuserDn(member))
	}

	err = state.user.CreateGroup(groupinfo)
	if err != nil {
		log.Println(err)
		http.Error(w, fmt.Sprint(err), http.StatusInternalServerError)
		return
...
func (state *RuntimeState) exitfromGroup(w http.ResponseWriter, r *http.Request) {
	...
	var groupinfo userinfo.GroupInfo
	groupinfo.Member = append(groupinfo.Member, state.user.CreateuserDn(username))
	groupinfo.MemberUid = append(groupinfo.MemberUid, username)
	for _, entry := range out["groups"] {
		groupinfo.Groupname = entry
		err = state.user.DeletemembersfromGroup(groupinfo)
		if err != nil {
			log.Println(err)
		}
	}
...
func (state *RuntimeState) pendingActions(w http.ResponseWriter, r *http.Request) {
	...
	for _, entry := range DBentries {
		...
		description, err = state.user.GetDescriptionvalue(groupName)
		if err != nil {
			log.Println(err)
			http.Error(w, fmt.Sprint(err), http.StatusInternalServerError)
			return
		...
		if state.user.IsgroupmemberorNot(groupName, username) {
...
func (state *RuntimeState) approveHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, entry := range userPair {
		if state.user.IsgroupmemberorNot(entry[1], entry[0]) {
			...
			groupinfo.Member = append(groupinfo.Member, state.user.CreateuserDn(entry[0]))
			err := state.user.AddmemberstoExisting(groupinfo)
			if err != nil {
				log.Println(err)
			}
			err = deleteEntryInDB(entry[0], entry[1], state)
...
func (state *RuntimeState) deleteGrouphandler(w http.ResponseWriter, r *http.Request) {
	...
	err = state.user.DeleteGroup(groupnames)
	if err != nil {
		log.Println(err)
